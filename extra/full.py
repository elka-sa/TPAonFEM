...
class FullFile(AnsysBinary):
    ...
    def dof_ref(self):
        ...
        if self._dof_ref is None:
            ...
            _, index, nref, dref = _binary_reader.sort_nodal_eqlv(
                self.neqn, neqv, ndof
            )
    ...
    """Modified by elka_sa 10/09/2025 -> New function called read_array()"""
    def read_array(self, filename, ptr, nterm, neqn, const=None, endian="<"):
        """
        Reads stiffness or mass matrices from ANSYS Fortran files (upper triangle).
        ...
        filename : str
            Full filename.
        ptr : int
            Pointer to start of block (units of 4-byte words, as in the original code).
        nterm : int
            Number of terms to read (upper-triangular nonzeros).
        neqn : int
            Number of equations (matrix size).
        const : any
            Unused (kept for API compatibility).
        endian : {'<','>'}
            Endianness for unpacking ('<' little-endian, '>' big-endian).
        ...
        dt_i4 = np.dtype(endian + "i4")
        dt_f8 = np.dtype(endian + "f8")
        
        with open(filename, "rb") as f:
            f.seek(ptr * 4)
            buf = f.read(24*int(neqn) + 12*int(nterm))  # 24*neqn + 12*nterm bytes
        
        rows = np.empty(nterm, np.int32)
        cols = np.empty(nterm, np.int32)
        data = np.empty(nterm, np.float64)
        
        loc = pos = 0
        for col in range(neqn):
            nitems = int(np.frombuffer(buf, dt_i4, count=1, offset=loc)[0]); loc += 8

            # try:
            idx = np.frombuffer(buf, dt_i4, count=nitems, offset=loc) - 1; loc += 4 * nitems
            # except ValueError:
            #     print(col)
            rows[pos:pos+nitems] = np.minimum(idx, col)
            cols[pos:pos+nitems] = np.maximum(idx, col)
        
            loc += 12
            data[pos:pos+nitems] = np.frombuffer(buf, dt_f8, count=nitems, offset=loc); loc += 8 * nitems
            loc += 4
        
            pos += nitems
        
        return rows[:pos], cols[:pos], data[:pos]
    ...
    def load_km(self, as_sparse=True, sort=False):
        ...
        if ntermK:
            ...
            # )
            krow, kcol, kdata = self.read_array(
                self.filename, ptrSTF, ntermK, self.neqn, const
            )
        else:
        ...
        if ntermM:
            ...
            # )
            mrow, mcol, mdata = self.read_array(
                self.filename, ptrMAS, ntermM, self.neqn, const
            )
        else:
        ...
        """Modified by elka_sa 24/06/2024"""
        # Start modification 24/06/2024
        ptrDMP = self._header["ptrDMP"]  # Location in file to damping matrix
        ntermD = two_ints_to_long(self._header["ntermDl"], self._header["ntermDh"]) #number of terms in damping matrix
        
        if ntermD:
            """Modified by elka_sa 10/09/2025 -> New function called read_array()"""
            # drow, dcol, ddata = _binary_reader.read_array(
            #     self.filename, ptrDMP, ntermD, self.neqn, const
            # )
            drow, dcol, ddata = self.read_array(
                self.filename, ptrDMP, ntermD, self.neqn, const
            )
        else:
            warnings.warn("Missing damping matrix")
            ddata = None
        # End modification 24/06/2024
        ...
        if np.any(const < 0):
            ...
            if ddata is not None:
                mask = ~np.logical_or(np.in1d(drow, remove), np.in1d(dcol, remove))
                drow = drow[mask]
                dcol = dcol[mask]
                ddata = ddata[mask]
            # End modification 24/06/2024
        ...
        if sort:  # make sorting the same as ANSYS rdfull would output
            ...
            if ddata is not None:
                drow = index[drow]
                dcol = index[dcol]
                drow, dcol = np.sort(np.vstack((drow, dcol)), 0)
            # End modification 24/06/2024
        ...
        if as_sparse:
            ...
            # Start modification 24/06/2024
            if ddata is not None:
                d = np.zeros((self.neqn,) * 2)
                d[drow, dcol] = ddata
            else:
                d = None
            # End modification 24/06/2024  
            
        return dof_ref, k, m, d
